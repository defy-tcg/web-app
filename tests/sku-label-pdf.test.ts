...
test("PDF contains vector QR modules inside the physical quiet zone, not a raster barcode", async () => {
  ...
  const translations = [...content.matchAll(/1 0 0 1 ([\d.]+) ([\d.]+) cm/g)]
    .filter(([, x, y]) => Number(x) !== 0 || Number(y) !== 0);
  ...
  const document = await PDFDocument.load(bytes);
  assert.equal(document.getPage(0).node.Resources()?.lookup(PDFName.of("XObject"))?.toString(), "<<\n>>");
});
...
test("Unicode names reach the rasterizer intact, are normalized, and reuse one embedded image across copies", async () => {
  ...
  ], 2, async (name) => { names.push(name); return { png: pixel, heightMm: 2.8 }; });
