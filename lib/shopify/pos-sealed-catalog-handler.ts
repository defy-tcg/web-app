...
async function readLookup(request: Request): Promise<Lookup> {
  ...
  if (Object.keys(data).length !== 2 || typeof data.game !== "string" || !["pokemon", "onepiece", "riftbound", "gundam", "magicthegathering"].includes(data.game)) {
    throw new InputError("Choose Pokémon, One Piece, Riftbound, Gundam, or Magic: The Gathering and a product name or catalog ID.");
  }
