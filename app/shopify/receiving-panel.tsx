...
export default function ReceivingPanel({ onRefreshed }: { onRefreshed?: () => void | Promise<void> }) {
  ...
  return <>
    <section className="shopify-panel shopify-receiving-guide" id="shopify-receive-stock" aria-labelledby="shopify-receiving-heading">
      <div className="shopify-receiving-heading"><div><p className="shopify-eyebrow">SHOPIFY POS · DEFY RECEIVING</p><h2 id="shopify-receiving-heading">Receive stock</h2><p>Use the Defy Receiving tile on your Shopify POS device. Each confirmed receipt adds stock to the Shopify receiving location.</p></div><span className="shopify-receiving-badge">Shopify stock</span></div>
      <ol className="shopify-receiving-steps">
        ...
        <li><strong>Enter quantity and cost</strong><p>Enter the units received and the cost per unit. Confirm the receiving location.</p></li>
        <li><strong>Save the receipt</strong><p>Tap <b>Save receipt</b> and wait for confirmation. Refresh the history below to load the confirmed receipt and saved stock.</p></li>
      </ol>
      <div className="shopify-receiving-followup"><p><strong>New product?</strong> Set its selling price and publish it to the <strong>Point of Sale</strong> sales channel in Shopify before selling. A new draft may already hold received stock.</p><Link href="/singles" className="shopify-button">Receive catalog singles</Link></div>
    </section>
    ...
    <section className="shopify-panel shopify-records shopify-receiving-history" aria-labelledby="shopify-receiving-history-heading" aria-busy={loading || loadingMore}>
      <header><div><h2 id="shopify-receiving-history-heading">Receiving history</h2><p>Confirmed Shopify receipts for the configured receiving location. Refresh history also reloads saved stock. If stock still looks old, use <strong>Sync now</strong> below to read the latest Shopify balances.</p></div><button type="button" className="shopify-button" disabled={loading || loadingMore} onClick={() => void load()}>{loading ? "Loading…" : "↻ Refresh history"}</button></header>
      ...
      {loading && !page ? <div className="shopify-empty" role="status">Loading receiving history…</div> : receipts.length ? <div className="shopify-table-scroll" tabIndex={0} role="region" aria-label="Confirmed Shopify receiving history; scroll horizontally for all columns">
        <table><thead><tr><th scope="col">Received product</th><th scope="col">SKU / barcode</th><th scope="col">Quantity</th><th scope="col">Unit cost</th><th scope="col">Total cost</th><th scope="col">Received</th></tr></thead>
          <tbody>{receipts.map((receipt) => <tr key={receipt.id}>
            <th scope="row"><strong>{receipt.name}</strong><small>{receipt.locationName || "Receiving location"} · Confirmed in Shopify</small>{receipt.createdProduct && <small>Product created with this receipt</small>}{receipt.staged && <small>Draft at receipt</small>}</th>
            <td className="shopify-sku">{receipt.sku || "SKU not set"}<small>{receipt.barcode ? `Barcode: ${receipt.barcode}` : "Barcode not set"}</small></td>
            <td><strong>+{quantity.format(receipt.quantity)}</strong><small>{receipt.unit}</small></td>
            <td>{cost(receipt.unitCostCents, receipt.currencyCode)}</td><td>{cost(receipt.totalCostCents, receipt.currencyCode)}</td>
            <td>{date(receipt.receivedDate)}<small>Confirmed {date(receipt.appliedAt, true)}</small></td>
          </tr>)}</tbody>
        </table>
      </div> : page && !error ? <div className="shopify-empty"><h3>No receipts on the loaded pages</h3><p>{page.hasMore ? "More history is available. Load more to find receipts for this location." : "Confirmed receipts will appear here after stock is received through Defy Receiving."}</p></div> : null}
